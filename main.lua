...
local function create_components()
  ...
  M.auto_loot = m.AutoLoot.new( M.loot_list, M.api, db( "auto_loot" ), M.config, M.player_info, M.game_api )
  ...
  M.roll_for_broadcast = m.RollForBroadcast.new( M.roll_controller, M.config, M.awarded_loot, M.player_info )
  M.roll_for_receiver = m.RollForReceiver.new( M.rolling_popup, db( "roll_for_receiver" ), M.winner_tracker, M.awarded_loot )
...
local function on_check_softres_command( args )
  if string.find( args, "^a" ) then
    ...
    if result == M.softres_check.ResultType.SomeoneIsNotSoftRessing and m.raid_id then
      local msg = string.format( "https://chromieres.com/%s", m.raid_id )
